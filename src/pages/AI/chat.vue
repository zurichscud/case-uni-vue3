...
function buildReplyData(message, payload) {
  try {
    const replyContent = payload.content || ''
      ...
      if (item.reference) {
        if (Array.isArray(item.reference)) {
          ...
          try {
                ...
                .map((ref) => {
                  ...
                  if (typeof ref === 'string') {
                    ...
                    try {
                      ...
                      console.warn('无法解析引用文献字符串:', ref, err)
                      return null
                    }
                  }
            ...
            console.warn('无法解析引用文献数据:', item.reference, err)
            references = []
          }
        }
      }
...
<template>
  ...
  <view class="main">
    ...
    <scroll-view scroll-y="true" class="scroll" :scroll-top="scrollTop" :scroll-with-animation="true" :style="{
      ...
      top: `calc(${menuButtonInfo.top + menuButtonInfo.height}px + 2rpx)`,
      height: `calc(100vh - ${menuButtonInfo.top + menuButtonInfo.height}px - 2rpx)`,
    }"
    >
              ...
              <view class="reference" v-if="item.msg.references && item.msg.references.length">
                ...
                <view class="reference-list">
                  <view class="reference-item" v-for="(re, idx) in item.msg.references" :key="idx">
                    ...
                    <text class="reference-index">
                      [{{ idx + 1 }}]
                    </text>
                    <text class="reference-name">
                      {{ re.name }}
                    </text>
                  </view>
                </view>
