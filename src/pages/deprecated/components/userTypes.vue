    ...
    >
      ...
      >
        ...
        <view class="" style="margin: -16rpx 10rpx 0 10rpx">
          ...
          <view
            :style="{
              backgroundColor: list.messageType != 3 ? 'white' : 'transparent'
            }"
            class="casek"
            :class="[
              list.type == 3 || list.type == 2 ? 'casek' : 'caseks',
              lists.messageType != 3 ? 'casek_padding' : ''
            ]"
          >
            <!-- <rich-text selectable  class='warpS' v-if="list.messageType==1||!list.messageType" selectable :nodes="lists.msg"></rich-text> -->
            <text user-select class="warpS" v-if="list.messageType == 1 || !list.messageType">
              {{ lists.msg }}
            </text>

            <image
              lazy-load
              style="width: 140rpx; height: 282rpx; border-radius: 10rpx"
              @click="openImage(list.msg)"
              :src="lists.msg"
              alt=""
              mode="aspectFill"
              v-else-if="list.messageType == 3"
            ></image>
            <view class="fileType" v-else-if="list.messageType == 2" @click="download(list.msg)">
              <view class="msgFile">
                {{ list.msg | fileUrl('name') }}
              </view>
              <view class="" style="flex: 1">
                <image
                  style="width: 88rpx; height: 88rpx; vertical-align: middle"
                  :src="list.msg | fileUrl('url')"
                  mode=""
                ></image>
              </view>
            </view>
            <view style="font-size: 24rpx" v-else-if="list.messageType == 4">
              <view class=""> 专家向您发出会议邀请 </view>
              <view class="btnhuiyi" @click="navTo(list.msg)"> 立即查看 </view>
            </view>
          </view>
        </view>
      </view>
    </view>
    <view v-if="list.messageType == 5">
