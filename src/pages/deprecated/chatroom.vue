    ...
    >
      <view class="total_y" ref="elements">
        <view v-for="(item, i) in dataList" :key="item.id" :id="'item' + item.id">
          ...
          >
            {{ item.createDate | futter(dataList[i == 0 ? 0 : i - 1].createDate, i, item) }}
          </view>
...
<script>
  ...
  export default {
    ...
    onLoad(query) {
      this.options = query
      this.getCaseInfo(query.caseId)
      uni.setNavigationBarTitle({
        title: '聊天室'
      })
      this.getList(query.caseId)
    },
    async onShow() {
      uni.hideLoading()
      await uni.onKeyboardHeightChange((res) => {
    ...
    methods: {
      ...
      async dataFilter() {
        let { datas } = await this.$api.getImage({
                ...
                success: (resImg) => {
                  const uploadTask = uni.uploadFile({
                    ...
                    success: async (uploadFileRes) => {
                      let res = await that.$api.photoImg({
            ...
            if (lexin == 'docx' || lexin == 'doc' || lexin == 'pdf') {
              ...
              const uploadTask = uni.uploadFile({
                ...
                success: async (uploadFileRes) => {
                  ...
                  let res = await that.$api.FilePhoto({
                    file: strFiles.url,
                    name: strname,
                    userId: that.user.id,
                    caseId: that.options.caseId
      ...
      // 第一次获取消息
      async getList(data) {
        const { rows, code, total } = await this.$api.chatWith(
          `?isAsc=desc&orderByColumn=id&pageNum=1&pageSize=20`,
          {
            caseId: data,
            userId: this.user.id
          }
      ...
      async listData(data) {
        // 获取当前total
        const res = await this.$api.listData({
          caseId: data
        })
      ...
      async getLists(pics, my) {
        const { rows, code, total } = await this.$api.chatWith(
          `?isAsc=desc&orderByColumn=id&pageNum=1&pageSize=${pics}`,
          {
            caseId: this.options.caseId,
            userId: this.user.id
          }
      ...
      upper: async function (e) {
        ...
        if (this.total > this.general) {
          ...
          const { rows, code, total } = await this.$api.pillList({
            caseId: this.options.caseId,
