<template>
  <view class="chat_bar clear">
      ...
      <view class="bottom">
        ...
        <block>
          ...
          <view
            v-if="isSubmit != 0"
            class="issubimt"
            style="height: 400rpx; border: 1px solid transparent"
          >
          ...
          >
            ...
            <view v-if="typeIndex == 2" class="input" :key="'inp' + orderIndex">
              ...
              <view class="topInput">
                <view
                  class="li"
                  style="width: 600rpx; margin: 0 auto 20rpx auto; white-space: pre-wrap"
                >
                  {{ inputStyle[inputIndex].nametext }}
                </view>
                <view style="width: 100%">
                  <input
                    :type="inputStyle[inputIndex].type"
                    :style="{ width: inputStyle[inputIndex].width }"
                    :maxlength="inputStyle[inputIndex].maxlength"
                  ...
                  />
                  <text
                    v-if="inputStyle[inputIndex].type == 'number'"
                    style="margin-left: 32rpx; vertical-align: middle"
                    >元</text
                  >
                </view>
              </view>
            ...
            <view
              v-if="typeIndex == 3"
              :key="'aswer' + orderIndex"
              class="pickerView finishBox"
            ...
            >
              ...
              >
                <picker-view-column style="transform: translateY(-75px)" class="view-column first">
                  <view
                    class="item"
                    :class="msgIndex == index ? 'colorType' : ''"
                    v-for="(item, index) in lastQ"
                    :key="index"
                    >{{ item.msg }}
                  </view>
                </picker-view-column>
            ...
            <view v-if="typeIndex == 4" class="input finishBox" style="text-align: center">
              <view class="submit_flex">
                <view class="submit1 lot" v-if="!flag" @click="reselection"> 重选 </view>
              ...
              <view class="singleBox1" v-if="flag">
                ...
                <view
                  :class="[item.checked ? 'colorR' : '', 'itPublic', 'item']"
                  v-for="(item, index) in lastQ"
                  :key="index"
                  @click="selectOptions(index, item)"
                >
                  {{ item.msg }}
                </view>
              </view>
    ...
    <alert ref="child" title="受理条件" @submit="nav('case')" closex="true" auto="false">
      <view class="alertC">
        <view class="colorAl alert"> 您的案件服务申请在满足以下条件时，理赔公社将予以受理 </view>
        <view class="colorAl_fo alert"> 1.重疾/防癌/其他特种疾病: </view>
        <view class="colorAl_fw alert"> 理赔争议金额≥10万元 </view>
        <view class="colorAl_fo alert"> 2.因意外或疾病导致的: </view>
        <view class="colorAl_fw alert"> ①医疗（报销/津贴费用）理赔争议金额≥5万元 </view>
        <view class=""> ②身故/残疾理赔争议金额≥10万元 </view>
        <view class="colorAl_btn" @click="$refs.child.close()"> 确定 </view>
      </view>
    </alert>
    <alert ref="child1" title="温馨提示" closex="true" auto="false">
      <view class="alert_style">
        <view style="width: 600rpx" class="child">
          <view class="alertC">
            <view class="text_pay"> 请确认上述案件真实有效 </view>
            <view class="text_pay"> 稍后有专家与你进一步沟通 </view>

            <view class="child_flex">
              <view class="child_btnerr" @click="navTab"> 返回首页 </view>
              <view class="child_btnnext" @click="submitCase()"> 确定 </view>
            </view>
          </view>
        </view>
      </view>
    ...
    <alert ref="child2" title="温馨提示" closex="true" auto="false">
      <view class="alert_style">
        <view style="width: 600rpx" class="child">
          <view class="text_pay"> 理赔金额2万元以下的<br />理赔公社提供案件咨询服务 </view>
          ...
          <view class="child_flex">
            <view class="child_btnerr" @click="$refs.child2.close()"> 取消 </view>
            <view class="child_btnnext" @click="goSeek"> 去咨询 </view>
          </view>
        </view>
      </view>
    </alert>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState(['user', 'chatMsg', 'questionId'])
    },
    watch: {},
    created() {
      let that = this
    ...
    methods: {
      ...
      ...mapMutations(['pushChatMsg', 'initChatMsg', 'setChatMsg']),
      // 初始化数据
      pageInit() {
      ...
      async submitCase() {
        ...
        msg.messageType = 5
        msg.msg = `<div style='font-size:12px;width: 100%;white-space: nowrap;color: #c5c5c5;text-align: center;margin: 20px 0;'><span style='color:#999;'>———————</span><span style='margin:0 20px;'>专家正在赶来的路上~</span><span style='color:#999;'>———————</span></div>`
        await this.addMsg(msg)
        uni.redirectTo({
      ...
      async submit() {
        ...
        if (!this.flay) {
          ...
          /* function Detail(){if (this.current == 2) {
						// 问题1

					} else if (this.current == 3) {
						// 问题二
						let answerId = this.judgeNext()
						...
						this.aniList.answer4 = answerId
						this.aniList.answer2ServiceItem = answerId
						...
						if (this.tiIndex == 0) {
							if (answerId == 24) {
								this.typeIndex = 3
								this.current = 50
								await this.getQuestionCurrent(this.current, this.awer1)
							} else if (answerId == 20 || answerId == 22) {
								this.typeIndex = 4
								if (answerId == 20) {
									this.pushChatMsg({
										answerType: 1,
										type: 3,
										msg: '<p>服务内容：对保险公司拒赔决定的合理性/案件诉讼风险/调解方案合理性进行评估。</p><br/><p>服务规则：仅限一家保险公司一起理赔案件。</p><br/><p>收费标准： 600元/次。</p><br/><p>服务时间：工作日9:00-18:00</p>'
									})
									msg.msg =
										'服务内容：对保险公司拒赔决定的合理性/案件诉讼风险/调解方案合理性进行评估。\n\n服务规则：仅限一家保险公司一起理赔案件。\n\n收费标准： 600元/次。\n\n服务时间：工作日9:00-18:00'
									msg.messageType = 1
									await this.addMsg(msg)
									...
									this.pushChatMsg({
										...
										type: 3,
										pdf: true,
										msg: '<div @click="naoTo" class="" style="display: flex;width:230px;justify-content: space-between;"><div class="" style="flex-grow: 1;">案件评估示例.pdf</div><div class=""><img style="width: 44px;height: 44px;"  src="https://app.y9net.cn/data/01/32/wKgBNmNbb8WAOJqTAAAKj1Rk9sM643.png" mode=""></img></div></div>',
										url: 'https://app.y9net.cn/data/01/3B/wKgBNmQmelaALJAeAAI6kdqXKXA867.pdf'
									})
									msg.msg =
										'https://app.y9net.cn/data/01/3B/wKgBNmQmelaALJAeAAI6kdqXKXA867.pdf,案件评估示例.pdf'
									msg.messageType = 2
									await this.addMsg(msg)
								} else {
									this.pushChatMsg({
								...
								await this.getQuestionCurrent(this.current, this.lastQ[this.msgIndex].answerId)
							...
							if (answerId == 19) {
								...
								msg.statu = 1
								msg.caseType = 1
								this.$api.updateCase(Object.assign(msg, this.aniList))
							}
						}
						this.msgIndex = 0
					} else if (this.current == 51) {
						let answerId = this.judgeNext()
						await this.pushChatMsg({
							answerType: 2,
							type: 1,
						...
						this.current = 3
						msg.type = "3"
						if (this.aniList.answer2ServiceItem == 32) {
							this.pushChatMsg({
								answerType: 1,
								type: 3,
								msg: '<p>服务内容：对保险公司拒赔决定提出异议代理意见。</p><br/><p>服务规则：仅限一家保险公司一起理赔案件。</p><br/><p>收费标准： 3000元/次。</p><br/><p>服务时间：工作日9:00-18:00</p>'
							})
							msg.msg =
								'服务内容：对保险公司拒赔决定提出异议代理意见。\n\n服务规则：仅限一家保险公司一起理赔案件。\n\n收费标准： 3000元/次。\n\n服务时间：工作日9:00-18:00'
							msg.messageType = 1
							await this.addMsg(msg)
							this.pushChatMsg({
								answerType: 1,
								type: 3,
								pdf: true,
								msg: '<div @click="naoTo" class="" style="display: flex;width:230px;justify-content: space-between;"><div class="" style="flex-grow: 1;">拒赔异议书示例.pdf</div><div class=""><img style="width: 44px;height: 44px;"  src="https://app.y9net.cn/data/01/32/wKgBNmNbb8WAOJqTAAAKj1Rk9sM643.png" mode=""></img></div></div>',
								url: 'https://app.y9net.cn/data/01/3B/wKgBNmQmerSAUDVTAAHdSC1qXnM961.pdf'
							})
							msg.msg = 'https://app.y9net.cn/data/01/3B/wKgBNmQmerSAUDVTAAHdSC1qXnM961.pdf,拒赔异议书示例.pdf'
							msg.messageType = 2
							await this.addMsg(msg)
						} else if (this.aniList.answer2ServiceItem == 33) {
							this.pushChatMsg({
								answerType: 1,
								type: 3,
								msg: '<p>服务规则：仅限一家保险公司一起理赔案件。</p><br/><p>收费标准： 2500元/次。</p><br/><p>服务时间：工作日9:00-18:00</p>'
							})
							msg.msg =
								'服务规则：仅限一家保险公司一起理赔案件。\n\n收费标准： 2500元/次。\n\n服务时间：工作日9:00-18:00'
							msg.messageType = 1
							await this.addMsg(msg)
						} else if (this.aniList.answer2ServiceItem == 34) {
							this.pushChatMsg({
								answerType: 1,
								type: 3,
								msg: '<p>场景：庭审结束后的七天内向法庭提交。</p><br/><p>目的：对庭审中的争议焦点、难点问题进行重点阐述，以便法庭采纳本方观点。</p><br/><p>收费标准： 5000元/次。</p><br/><p>服务时间：工作日9:00-18:00</p>'
							})
							msg.msg =
								'场景：庭审结束后的七天内向法庭提交。\n\n目的：对庭审中的争议焦点、难点问题进行重点阐述，以便法庭采纳本方观点。\n\n收费标准： 5000元/次。\n\n服务时间：工作日9:00-18:00'
							msg.messageType = 1
							await this.addMsg(msg)
						}
						await this.getQuestionCurrent(this.current, this.lastQ[this.msgIndex].answerId)
						...
						if (!!this.lastMsg) {
							...
							await this.addMsg(Object.assign(msg, this.aniList))
							let answer3 = this.aniList.answer3InsuranceCategory
							if (answer3 == 10 ? this.lastMsg * 1 < 50000 : this.lastMsg * 1 < 100000) {
								msg.type = 3
								await this.pushChatMsg({
									answerType: 2,
									type: 3,
									msg: '您提交的案件不符合理赔公社的受理条件，请重新选择服务项目'
								})
								msg.msg = '您提交的案件不符合理赔公社的受理条件，请重新选择服务项目'
								await this.addMsg(Object.assign(msg, this.aniList))
								this.typeIndex = 3
								this.current = 2
								this.msgIndex = 0
								...
								if (this.user.remark != 5) {
									if (this.aniList.answer2ServiceItem == 21 || this.aniList.answer2ServiceItem ==
										23) {
										data.serviceType = this.aniList.answer2ServiceItem == 21 ? 7 : 8
										let res = await this.$api.getBonus(data)
										await this.pushChatMsg({
											answerType: 2,
											type: 4,
											msg: `<div style='font-size: 12px;color:#999999;text-align: center;'>可预期最大收入：基本费奖励：<span style='color:#3F9CFF;'>${res.data.basicFee}元</span>/增值费奖励：<span style='color:#3F9CFF;'>${res.data.increaseFee}元</span></div>`
										})
										msg.type = 3
										msg.msg =
											`<div style='font-size: 12px;color:#999999;text-align: center;'>可预期最大收入：基本费奖励：<span style='color:#3F9CFF;'>${res.data.basicFee}元</span>/增值费奖励：<span style='color:#3F9CFF;'>${res.data.increaseFee}元</span></div>`
										msg.messageType = 5
										await this.addMsg(Object.assign(msg, this.aniList))
									} else {}
								}
								await this.getQuestionCurrent(this.current)
								this.inputIndex = 1
							}
						} else {
							this.$toast('输入金额不能为空')
						...
						if (!!this.lastMsg) {
							...
							this.aniList.answer5 = this.lastMsg
							this.aniList.answer5Name = this.lastMsg
							await this.addMsg(Object.assign(msg, this.aniList))
							await this.getQuestionCurrent(this.current)
						} else {
							this.$toast('输入值不能为空')
						...
						if (!!this.lastMsg.trim()) {

						} else {
							this.lastMsg = "北京市北京市东城区"
						}
						await this.pushChatMsg({
							answerType: 2,
							type: 1,
						...
						msg.msg = this.lastMsg
						this.aniList.answer6 = this.lastMsg
						this.aniList.answer6CardAddress = this.lastMsg
						await this.addMsg(Object.assign(msg, this.aniList))
						await this.getQuestionCurrent(this.current)
						...
						msg.msg = this.lastMsg
						this.aniList.answer7 = this.lastMsg
						this.aniList.answer7RealAddress = this.lastMsg
						await this.addMsg(Object.assign(msg, this.aniList))
						await this.getQuestionCurrent(this.current)
					} else if (this.current == 9) {

					} else {
						if (!!this.lastMsg && this.current == 10) {
							await this.pushChatMsg({
								answerType: 2,
								type: 1,
								msg: this.lastMsg
							...
							msg.msg = this.lastMsg
							this.aniList.answer9 = this.lastMsg
							this.aniList.answer9RefusalReason = this.lastMsg
							await this.addMsg(Object.assign({}, msg, this.aniList))

							msg.statu = 1
							if (this.aniList.answer2ServiceItem == 21 || this.aniList.answer2ServiceItem == 23) {
								msg.caseType = 0
								await this.$api.updateCase(Object.assign(msg, this.aniList))
								msg.messageType = 5
								msg.msg =
									`<div style='font-size:12px;width: 100%;white-space: nowrap;color: #c5c5c5;margin: 20px 0;'><span style='color:#999;'>———————</span><span style='margin:0 20px;'>专家正在赶来的路上~</span><span style='color:#999;'>———————</span></div>`
								await this.addMsg(msg)
								uni.redirectTo({
									url: `/pages/deprecated/chatroom?caseId=${this.caseId}&userId=${this.user.id}`
								});
      ...
      nav(name) {
        uni.navigateTo({
          url: `/pages/case/case?orderId=${this.basic.caseId}`
        })
      },
      //获取聊天室的 输入框的高度
      getChatInputH() {
